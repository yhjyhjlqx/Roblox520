...
show_header() {
    clear
    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN} Roblox520 Hosts 更新工具 - Linux版${NC}"
    echo -e "${GREEN}========================================${NC}"
...
check_dependencies() {
    ...
    if [ ${#missing[@]} -ne 0 ]; then
        echo -e "${YELLOW}警告: 缺少必要工具: ${missing[*]}${NC}"
        echo -n "尝试安装... "
        
        if command -v apt-get &> /dev/null; then
            apt-get update -qq && apt-get install -yq "${missing[@]}" >> "$LOG_FILE" 2>&1
...
download_package() {
    ...
    if command -v curl &> /dev/null; then
        if curl -sL "$DOWNLOAD_URL" -o "$ZIP_PATH" >> "$LOG_FILE" 2>&1; then
            echo -e "${GREEN}完成(curl)${NC}"
        else
            echo -e "${RED}失败${NC}"
            exit 1
        fi
    elif command -v wget &> /dev/null; then
        if wget -q "$DOWNLOAD_URL" -O "$ZIP_PATH" >> "$LOG_FILE" 2>&1; then
            echo -e "${GREEN}完成(wget)${NC}"
        else
            echo -e "${RED}失败${NC}"
            exit 1
        fi
...
flush_dns() {
    ...
    if systemctl is-active --quiet nscd; then
        systemctl restart nscd >> "$LOG_FILE" 2>&1
        ...
        echo -e "${YELLOW}警告: 未找到已知的DNS缓存服务${NC}"
        return
    fi
    echo -e "${GREEN}完成${NC}"
}
